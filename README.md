...
```txt
<p id="show">아무것도 안적혔다.</p>
p(element), id(attribute), 아무것도 안적혔다.(content)
```
...
// Doucment 객체의 getElementById("show")는 웹페이지에서 아이디 "show"
// 즉 <p> 요소를 가져온다.
// innerHTML 에 "안녕!"을 설정함으로써
// 단락 p 의 내용을 "안녕" 으로 변경
// DOM method는 getElementById()
// DOM property는 innerHTML
document.getElementById("show").innerHTML = "안녕!";

// 문서가 로드될 때, 글 제목 요소 h1을 생성하는 예제
...
```js
function changeColor(글자색상) {
  // document.getElementById("p1").style.color = 글자색상;
  const 요소 = document.getElementById("p1");
  요소.style.color = 글자색상;
}
```
...
// All이라서 항목이 하나만 있어도 몇번째인지 적어줘야함
...
alert(x.innerHTML); // <span>안녕</span>
...
```html
<body>
  <!-- form 태그 안에 버튼이있는경우 버튼타입은 자동으로 서브밋이 됩니다 -->
  <form id="form1">
